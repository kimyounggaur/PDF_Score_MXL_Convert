...
error           text
cost_usd        numeric default 0                 -- Claude 호출 누적 비용(정확값)
cost_breakdown  jsonb                             -- 모델별 분해 {sonnet, opus, total}
```
...
CREATE INDEX idx_cost_log_job ON api_cost_log(job_id, created_at);
...
### Phase 7.5 — 실시간 비용 추적 · 대시보드

> Audiveris는 무료지만 Claude API는 실제 비용 항목입니다. 호출마다 토큰·비용을 기록해
> 잡 화면에 실시간 표시하고, 상한 초과를 차단하며, 일/월 단위로 집계합니다.

```text
[프롬프트 — Phase 7.5]

목표: 모든 Claude 호출의 토큰·비용을 호출 직후 기록하고, 실시간 조회/표시/상한차단/일일집계를 구현한다.

전제(정확성):
- 비용은 추정이 아니라 API 응답의 usage(input_tokens, output_tokens)로 정확히 계산한다.
- 단가는 코드 상수로 두되, 공식 문서(platform.claude.com/docs pricing)를 기준으로 관리하고
  주석에 "확인일자"를 남긴다. 단가는 바뀔 수 있으므로 하드코딩 값을 맹신하지 말 것.
- 현재 단가(2026-06 확인, per 1M tokens):
    claude-sonnet-4-6 : input $3 / output $15
    claude-opus-4-8   : input $5 / output $25
  (구형 Claude 3 Opus의 $15/$75와 혼동 금지)
- 프롬프트 캐싱을 쓰면 usage에 cache_creation_input_tokens / cache_read_input_tokens가 추가된다.
  캐시 사용 시 이 필드도 각각의 단가(쓰기 1.25x, 읽기 ~0.1x)로 비용에 반영한다.

요구사항:
1) 비용 계산 모듈 (cost.ts)
   - PRICING 상수표 + calculateCost(model, usage) → { input, output, total }
   - 캐시 토큰 필드가 있으면 함께 합산.

2) 호출 시 즉시 기록 (vision.ts 확장)
   - verifyPage가 응답의 usage를 읽어 비용 계산.
   - api_cost_log에 1행 insert (jobId, pageNum, model, 토큰, cost).
   - jobs.cost_usd를 원자적으로 누적 증가(증가분만 더함), cost_breakdown(jsonb: {sonnet, opus, total}) 갱신.

3) 상한 차단
   - 누적이 JOB_COST_LIMIT_USD 초과 시: jobs.status='failed', error 기록,
     (선택) 관리자 알림(Slack/이메일), 그리고 직전까지의 결과를 사용자에게 반환.
   - REFINE 루프(P4.5) 안에서도 매 패스 전에 상한을 선검사해 추가 호출을 막는다.

4) 실시간 조회 API
   - GET /api/jobs/:id/cost → { totalCost, breakdown, pageLog[] }
   - pageLog는 created_at desc, 페이지별 (model, 토큰, cost).

5) 프론트 실시간 표시 (CostTracker 컴포넌트)
   - 진행 화면에서 2초 폴링으로 누적 비용·모델별 분해·페이지별 상세(접이식) 표시.
   - 상한 근접 시(예: 80%) 경고 색으로 표시.

6) 일일 집계 (선택, 야간 배치 또는 트리거)
   - daily_cost_summary 갱신(date, total_jobs, total_cost, sonnet_cost, opus_cost).
   - 관리자 대시보드(/cost): 오늘 누적·잡수·평균비용 KPI + 최근 30일 추이 차트.

산출물:
- /worker/src/cost.ts          (PRICING + calculateCost)
- /worker/src/vision.ts        (호출 직후 기록·누적·상한)
- /app/api/jobs/[id]/cost/route.ts
- /components/CostTracker.tsx
- /app/(admin)/cost/page.tsx   (선택)
- 마이그레이션: jobs.cost_usd / cost_breakdown, api_cost_log, daily_cost_summary

테스트:
- usage 목 주입 → 비용 계산 정확성(캐시 필드 포함)
- 상한 초과 시 failed 전환 + 직전 결과 반환
- 동시 호출에서 cost_usd 누적이 어긋나지 않음(원자적 증가 검증)
```

**완료 판정**
- 잡 진행 중 화면에서 누적 비용이 실시간으로 오름
- 상한 초과 시 추가 호출 없이 차단되고 직전 결과 반환
- (선택) 관리자 대시보드에서 오늘/30일 비용 확인

> **비용 절감 팁(선택 적용)**: 이 파이프라인은 비동기라 **Batch API(50% 할인)** 적용 여지가 크고,
> 페이지마다 동일한 지시문을 보내므로 **프롬프트 캐싱(캐시 읽기 ~90% 할인)** 으로 반복 입력 비용을
> 크게 줄일 수 있습니다. 둘 다 쓰면 Claude 비용을 추가로 절반 이하로 낮출 수 있어요.
...
- **비용**: Audiveris는 무료. Claude만 과금되며 페이지×모델 단가로 계산. 현재 단가(2026-06): Sonnet 4.6 $3/$15, Opus 4.8 $5/$25 (per 1M in/out). 전 페이지 sonnet → 저신뢰 페이지만 opus 티어링으로 최적화. 잡당/일일 상한·**실시간 정확 집계(Phase 7.5)** 필수. 비동기 특성상 Batch API(50%)·프롬프트 캐싱(캐시 읽기 ~90%)으로 추가 절감 가능.
...
- [ ] Phase 7.5: 실시간 비용 표시 + 상한 차단 + (선택) 대시보드
